...
AC_DEFUN([TEA_ENABLE_SHARED],[
	...
	SHARED_BUILD=0
	AC_DEFINE([STATIC_BUILD], [1], [Defined to 1 if doing a static build])
    fi
...
AC_DEFUN([TEA_ENABLE_THREADS], [
	...
	AC_DEFINE([TCL_THREADS], [1], [Defined to 1 if using threads])
	#LIBS="$LIBS $THREADS_LIBS"
	AC_MSG_RESULT([yes])
    else
...
AC_DEFUN([TEA_ENABLE_SYMBOLS], [
	...
	AC_DEFINE([TCL_MEM_DEBUG], [1], [Defined to 1 if enabled symbols for mem debugging])
    fi
...
AC_DEFUN([TEA_CONFIG_CFLAGS],[
    ...
    AC_ARG_ENABLE([64bit],[AS_HELP_STRING([--enable-64bit],[enable 64bit support (where applicable)])], [do64bit=$enableval], [do64bit=no])
    AC_MSG_RESULT([$do64bit])
 
    # Step 0.b: Enable Solaris 64 bit VIS support?

    AC_MSG_CHECKING([if 64bit Sparc VIS support is requested])
    AC_ARG_ENABLE([64bit-vis],[AS_HELP_STRING([--enable-64bit-vis],[enable 64bit Sparc VIS support])], [do64bitVIS=$enableval], [do64bitVIS=no])
	...
	AC_ARG_ENABLE([wince],[AS_HELP_STRING([--enable-wince],[enable Win/CE support (where applicable)])], [doWince=$enableval], [doWince=no])
	...
	windows)
	    ...
	    if test "$GCC" != "yes" ; then
		    ...
		    for i in $defs ; do
			AC_DEFINE_UNQUOTED([$i], [1], [Defined to 1 if there is something about $i that needs to be defined])
		    done
		    AC_DEFINE_UNQUOTED([_WIN32_WCE], [$CEVERSION], [Version of WINCE])
		    AC_DEFINE_UNQUOTED([UNDER_CE], [$CEVERSION], [Version of WINCE])
	...
	HP-UX-*.11.*)
	    ...
	    AC_DEFINE([_XOPEN_SOURCE_EXTENDED], [1], [Defined to 1 if we are going to use updated header definitions where possible])
	...
	OS/390-*)
	    ...
	    AC_DEFINE([_OE_SOCKETS], [1], [Apparently this define is needed in sys/socket.h])  # needed in sys/socket.h
	    ;;      
...
AC_DEFUN([TEA_PATH_X],[
	...
	case ${TK_DEFS} in
	    *MAC_OSX_TK*)
		AC_DEFINE([MAC_OSX_TK], [1], [Defined to 1 if using aqua interface for Tk])
...
AC_DEFUN([TEA_TIME_HANDLER],[
	...
	AC_DEFINE([HAVE_TM_TZADJ], [1], [Defined to 1 if we have tm_tzadj in struct tm])
    fi
	...
	AC_DEFINE([HAVE_TM_GMTOFF], [1], [Defined to 1 if we have tm_gmtoff in struct tm])
    fi
...
AC_DEFUN([TEA_TCL_LINK_LIBS], [
    ...
    AC_CHECK_HEADER([net/errno.h], [AC_DEFINE([HAVE_NET_ERRNO_H], [1], [Defined to 1 if we have net/errno.h])])
...
AC_DEFUN([TEA_TCL_EARLY_FLAG],[
	...
	AC_DEFINE([$1], [1], [Defined to 1 if $1 is required])
...
AC_DEFUN([TEA_TCL_64BIT_FLAGS],[
	...
	AC_DEFINE_UNQUOTED([TCL_WIDE_INT_TYPE],[${tcl_cv_type_64bit}], [wide int type])
...
AC_DEFUN([TEA_INIT],[
    ...
    if test x"${PACKAGE_NAME}" = x ; then
	    AC_MSG_ERROR([The PACKAGE_NAME variable must be defined by your TEA configure.ac])
    fi
    if test x"$1" = x ; then
	    AC_MSG_ERROR([TEA version not specified.])
...
#------------------------------------------------------------------------
    ...
    if test "${TEA_PLATFORM}" = "unix" ; then
        test ! -z "$CC"
	    AC_REQUIRE([TEA_TCL_LINK_LIBS])
	    AC_REQUIRE([TEA_MISSING_POSIX_HEADERS])
    fi
])
...
AC_DEFUN([TEA_PATH_CONFIG],[
    #
    # Ok, let us find the $1 configuration
    # First, look for one uninstalled.
...
AC_DEFUN([TEA_PATH_CELIB],[
	...
	no_celib=true
	AC_ARG_WITH([celib],[AS_HELP_STRING([--with-celib=DIR],[use Windows/CE support library from DIR])], [with_celibconfig=${withval}])
